...
bool LCS::setXY()
{
    ...
    catch (vector<char> texts)              // ACGT 예외처리
    {
        for (auto &text: texts)
            cout << text << ' ';
    ...
    catch (unsigned long long int length)   // 문자 길이 예외처리
    {
        cout << "입력하신 문자열의 길이는 " << length <<"자입니다." << endl;
        cout << "입력 문자열은 최소 10자 이상이여야 합니다.";
        return false;
    }
